...
// newPage 从浏览器池里借一个实例并开页面，省掉每请求约 1.5~2 秒的 Chromium 冷启。
// 见 browser/pool.go。返回的 release 必须 defer 调用，否则池子会漏空。
// 只有登录二维码那条路径还用 newBrowser()——它要让浏览器活过异步等待，
// 生命周期跟请求对不上。
func newPage() (*rod.Page, func()) {
	return browser.NewPage(configs.IsHeadless(), browser.WithBinPath(configs.GetBinPath()))
}
